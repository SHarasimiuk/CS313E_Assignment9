...
def step_size(s):
    ...
    for c in s:
        letter = ord(c) - 96
        step = (step * 26 + letter) % STEP_SIZE_CONSTANT
    return STEP_SIZE_CONSTANT - step
...
def insert_word(s, hash_table):
    ...
    hash_index = hash_word(s, table_size)
    step = step_size(s)
    if hash_table[hash_index] == "" or hash_table[hash_index] == s:
        hash_table[hash_index] = s
    else:
        while hash_table[hash_index] != "" and hash_table[hash_index] != s:
            hash_index = (hash_index + step) % table_size
        hash_table[hash_index] = s
...
def is_reducible(s, hash_table, hash_memo):
    ...
    for c in range(len(s)):
        new = s[:c] + s[c + 1:]
        if find_word(new, hash_table):
            if is_reducible(new, hash_table, hash_memo):
